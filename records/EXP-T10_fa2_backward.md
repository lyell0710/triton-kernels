...
## 1. 目的与假设
补上 FA2 的 backward:假设(跑前锁)**三梯度 dq/dk/dv 的 max abs err <2e-2**(与 forward gate 同口径:fp16 输入/fp32 累加/对 torch autograd 的朴素 fp32 反向比对),且**S=4096 效率 ≥SDPA-flash backward 的 70%**(参照 forward 的 87% 留余量)。
...
## 3. 步骤
`scripts/test_fa2_bwd.py`(6 正确性形状 + 4 序列长 bench ×3 轮)→ 聚合 `data/derived/exp-t10_stability_3rounds.csv`。
...
**FLOPs 口径**:backward ≈ 8·S²·D per head(重算 S 2 + dP 2 + dQ/dK 4),causal 减半;S=4096 时本实现 **66.5 TFLOPS**。

## 6. 分析与结论
**两条假设均成立**,但性能呈现**明显的尺寸依赖**,值得写清:
- **S≥2048(稳定区)**:87.8% / 91.2%,轮间 std ≤0.3% —— 这是可对外引用的数字,与 forward 的 87% 同量级。
- **S≤1024(不稳区)**:512 时 214%(反超 2.1×)、1024 时 118% 但 std 6%。**不主张"小尺寸反超"**:同一次实验的单轮跑里 SDPA 的 512 值是 0.2809 ms,3 轮里却是 0.4312±0.0010 —— SDPA 在短序列下的后端选择/计时本身不稳,本实现侧反而稳定(±0.2%)。诚实结论:**短序列区间两者不可比,本实现只保证自己可复现**。
- **为什么 backward 的 TFLOPS(66.5)远低于 forward(123)**:backward 每 FLOP 要配套读更多数据(重算要再读一遍 Q/K/V/dO),算术强度低于 forward,更接近带宽侧 —— 这正是 recomputation 的代价,不是缺陷。
...
- **方法论沉淀**:bug#3 的"单边正确"指纹 + "kernel/手写公式/autograd 三方并排"排查法,建议进 `docs/theory/01_flashattention.md` 的调试小节。
- 红线:短序列(≤1024)的对比数字**不进对外文本**(SDPA 侧不稳,不可比)。
