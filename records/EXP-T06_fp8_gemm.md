...
## 1. 目的与假设
DeepGEMM 缩放策略(W 128×128 块 scale + A per-token-group)在 Ada mma
路线落地。假设(跑前锁):fp8 vs fp16 cuBLAS ≥1.3×;kernel 精确性
(vs 反量化参考)≤1e-3 相对。

## 2. 环境与配置
src/fp8_gemm.py:e4m3(满量程 448),BLOCK_K=128=缩放组、BLOCK_N=128=
权重块(sb 取标量的前提);反量化融合进 fp32 累加,不物化 fp16 权重。
正确性双层:①kernel vs 逐块反量化 fp32 精算;②量化保真 vs 原 fp16。

## 3. 步骤
scripts/test_fp8_gemm.py:两形状 × {fp8 prequant / fp8 在线量化端到端 /
fp16 triton / fp16 cuBLAS}。
...
在线量化端到端 72.8/64.4 TFLOPS(torch 侧量化 kernel 是瓶颈,仅示成本;
真实 serving 权重预量化、激活量化融合上游)。
