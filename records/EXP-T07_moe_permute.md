...
- gather 式换掉 scatter-add：无原子、数值顺序确定（与 EXP-017《D5 EPLB gate》的重排数值教训呼应：能选顺序确定的实现就选）。
...
## 8. 下游影响
阶段二 MoE Permute 项闭环；与 EXP-014《D1 MoE decode 分解》(fused_moe 56.4%)拼成完整 "MoE 层内时间去哪了"图景。
