...
## 1. 目的与假设
我实现了按专家排序的 permute 与加权收回的 unpermute。跑前假设有三条：Triton 数据搬运 ≥ torch 参考；往返一致性落在 bf16 噪声级；无原子实现可行。
...
## 3. 步骤
跑 scripts/test_moe_permute.py：往返一致性 + 专家分段单调性断言 + 三段 bench。
...
## 6. 分析与结论
- unpermute 的 12.5× 来自融合：torch 路径是四趟 kernel 加中间量，Triton 是单 kernel 一读一写。这是 theory/03 第三层的又一实例。
- gather 式换掉了 scatter-add：无原子，数值顺序确定。这与 vllm/experiments#EXP-017《D5 EPLB gate》的重排数值教训呼应——能选顺序确定的实现就选。
- **索引构建成了大头（0.27ms > 两次搬运之和）**，这就是 vLLM 给它写专用 CUDA kernel（moe_align_block_size）的实证理由。专用化记入 backlog。

## 7. 异常、偏差与开放问题
kernel 级 bench 未存逐轮 raw，json 里只存均值。未做 padding 对齐 grouped GEMM 的 block 边界，接真实 fused_moe 时需要。跨卡版（all-to-all）不做，阶段三读 DeepEP 的切入问题已在 theory/07 列出。

- backlog（2026-08-24 审计）：本记录与 README 引用的关键数字都是**单轮** bench，等 GPU 空闲再补 ≥3 轮 stability（mean/std 落 stability 文件）。

## 8. 下游影响
阶段二 MoE Permute 项闭环。它与 vllm/experiments#EXP-014《D1 MoE decode 分解》（fused_moe 56.4%）拼成完整的「MoE 层内时间去哪了」图景。
...
本记录 §5 的 unpermute（gather 式无原子、每 token 收 topk 行加权）曾被登记为「MoE unpermute/moe_align 上游 PR」的候选。我按「先查重」纪律复核后否决了它，理由两条，都是实测或实读代码得到的。

**(1) vLLM 已有同设计的专用 CUDA kernel，本实现是「重新发明」而非「填补空白」。**
...
也就是说，**「gather 式、无原子、每 block 负责一个 token 的全部 k 次求和」正是 vLLM 现网采用的设计**。§5 的 12.5× 是对 **torch 参考**（argsort + scatter-add）的，不是对 vLLM 现有 kernel；把它当成「上游可改进项」是口径错位。
...
**结论**：本仓 EXP-T07 的成果**留在本仓，作为「独立复现了一个工业级设计」的证据**——这对面试是加分项，说明设计判断与库作者一致——**但不作为上游 PR 立项**。§7 原有的「索引构建 0.27ms 超过两次搬运之和」的观察仍有价值：若要找上游增量，方向应是**索引构建（argsort/bincount）的 kernel 化**，而不是搬运本身；该方向也需先查是否有 PR 覆盖。

**对简历的影响（已核对）**：简历写的是「为 MoE unpermute 设计 gather 式无原子方案替代 scatter-add，快 **torch 参考** 12.5×」。这个口径本身正确，因为明确写了对照物是 torch 参考，不构成夸大。但**面试需主动说明**「该设计与 vLLM 现网 CUDA 实现一致，我是独立复现而非首创」，免得被追问时被动。
