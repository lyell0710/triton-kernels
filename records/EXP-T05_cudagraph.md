...
假设:每调用成本塌缩到 kernel 本体量级(消掉 ~30µs Python 分发)。

## 2-3. 配置与步骤
scripts/test_cudagraph.py:eager 循环 vs torch.cuda.graph 捕获重放,
triton 与 torch.softmax 双对照;预热跑 3 次(JIT+分配器)后捕获。
...
## 6. 分析与结论
假设成立且超预期:graph 重放把 Python/C++ 分发全部变成 graph 节点,
