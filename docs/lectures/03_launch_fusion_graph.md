...
> 读者：准备校招面试的作者本人，以及被"小 kernel 慢了 4 倍"卡住的工程师。读法：不跳步。每个论断后面都跟着它的证据锚（EXP 编号 / 文件：行号 / raw 路径）；所有数字与仓内现行口径逐字一致，来源见 records/ 与 data/derived/。引用规范：凡属论文/官方文档的论断，给出标题 + arXiv/DOI 编号 + 章节编号（文档给 URL 路径 + 小节名）；凡属本讲义补出的推导或折算，行内标注「本讲义推导」；无法用检索确认的说法标注「未核实」。
...
- [2. 直觉与第一性原理](#2-直觉与第一性原理)
  - [2.1 一次 kernel 调用到底花在哪](#21-一次-kernel-调用到底花在哪)
  - [2.2 没有这层区分会怎样](#22-没有这层区分会怎样)
  - [2.3 稳态、首次调用与"为什么 N=100 就够"](#23-稳态首次调用与为什么-n100-就够)
  - [2.4 三条贯穿全篇的公理](#24-三条贯穿全篇的公理)
- [3. 完整推导与机制](#3-完整推导与机制)
  - [3.1 第一层:带宽主导尺寸下,设备侧同速](#31-第一层带宽主导尺寸下设备侧同速)
  - [3.2 第二层:三点法把时间从 kernel 里赶出来](#32-第二层三点法把时间从-kernel-里赶出来)
  - [3.3 第三层:端到端反转,融合数比单核快慢更重要](#33-第三层端到端反转融合数比单核快慢更重要)
  - [3.4 第四层:CUDA Graph 把 launch 归零](#34-第四层cuda-graph-把-launch-归零)
  - [3.5 四层合起来:一棵可以当场画的决策树](#35-四层合起来一棵可以当场画的决策树)
  - [3.6 硬件与运行时语义层:这些结论被什么定死](#36-硬件与运行时语义层这些结论被什么定死)
  - [3.7 每个魔法数的来历(理论上界 / 硬件约束 / 实测扫描)](#37-每个魔法数的来历理论上界--硬件约束--实测扫描)
- [4. 代码逐段走读:行核、绑定层与 graph 捕获](#4-代码逐段走读行核绑定层与-graph-捕获)
- [5. 实验数据怎么读](#5-实验数据怎么读)
  - [5.1 fig3:launch 四口径(对数轴)](#51-fig3launch-四口径对数轴)
  - [5.2 EXP-T03 的五行表怎么读](#52-exp-t03-的五行表怎么读)
...
「Triton 比 CUDA 慢吗」是本仓明令**禁止裸答**的问题（本仓措辞约定）。原因不是政治正确，而是这句话在本仓的数据里同时有三个互相矛盾的答案。读完你应当能：
