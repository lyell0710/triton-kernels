...
「什么时候该用 CUDA」判断曲线的第三个点:
  - 计算主导的 GEMM:手写 CUDA wmma 够到真 cuBLAS 的 86%(Kernel_Optimazation#EXP-K02（CUDA Tensor Core GEMM 版本梯）)
  - 融合型 attention:同一套 wmma 只够到自家 Triton 的 28%(Kernel_Optimazation#EXP-K03（CUDA FA2 forward 简化版版本梯）)
